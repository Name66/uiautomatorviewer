...
//                    mWorkingNode = new RootWindowNode(attributes.getValue("windowName"), rotation);
                    /** google default end*/
                    // Always use rotation 0 to avoid rotation issue --brian 2018-9-1
            ...
            public void endElement(String uri, String localName, String qName) throws SAXException {
                ...
                if (mParentNode != null) {
                    ...
                    /**
                     * Added by brian 2018-8-22
                     * UiAutomator2.0+ add depth method to find object
                     */
                    //start
                    for (BasicTreeNode node : mNodeList) {
                        for (int i = 0; i < mNodeList.size(); i++) {
                            if (node.getParent() != null && !node.getAttributes().containsKey("depth")) {
                                node.getAttributes().put("depth", String.valueOf(i));
                            }
                            if (node.hasChild()) {
                                for (int j = 0; j < node.getChildCount(); j++) {
                                    node.getChildren()[j].getAttributes().put("depth", String.valueOf(Integer.valueOf(node.getAttributes().get("depth")) + 1));
                                }
                            }
                        }
                    }
